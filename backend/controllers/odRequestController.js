...
/**
 * Helper to generate Secure OD ID and PDF (Strict Compressed Image Match Layout)
 */
const generateODPDFHelper = async (odRequest) => {
    ...
    // --- Title ---
    doc.y = 110;
    doc.fillColor('#111827').font('Helvetica-Bold').fontSize(14).text('STUDENT OD APPROVAL MEMORANDUM', 50, 110, { align: 'center' });
    ...
    const rows = [
        ...
        ['Student Email:', odRequest.studentEmail],
    ...
    doc.fillColor('#111827').font('Helvetica-Bold').fontSize(11).text(odRequest.forwardedBy || 'Department Staff', textStartX + labelWidth, approvalBoxY + 15);

    doc.fillColor('#4B5563').font('Helvetica').fontSize(10).text('HOD Status:', textStartX, approvalBoxY + 40);
    doc.fillColor('#2563EB').font('Helvetica-Bold').fontSize(22).text('APPROVED', textStartX + labelWidth, approvalBoxY + 35, { underline: true });
